...
from SASRecModules_RETAIL import *
# import trfl
from utility_RETAIL import *
...
def parse_args():
    ...
    parser.add_argument('--data', nargs='?', default='data/retailrocket/raw',
                        help='data directory')
...
class GRU_PRLnetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, max_step):
        ...
        self.inputs = tf.compat.v1.placeholder(tf.int32, [None, state_size], name='inputs')
        # self.cls=tf.compat.v1.placeholder(tf.int32, [None], name='cls')
        self.len_state = tf.compat.v1.placeholder(tf.int32, [None], name='len_state')
        self.target = tf.compat.v1.placeholder(tf.int32, [None], name='target')  # target item, to calculate ce loss
        self.reward_to_go = tf.compat.v1.placeholder(tf.float32, [None], name='reward_to_go')
        self.timestamp = tf.compat.v1.placeholder(tf.int32, [None], name='timestamp')
        ...
        self.output =tf.compat.v1.layers.dense(self.state_final, self.item_num, activation=None)
    ...
    def initialize_embeddings(self):
        ...
        step_embeddings = tf.Variable(tf.random.normal([self.max_step, self.hidden_size], 0.0, 0.01),
                                      name='step_embeddings')
        reward_embedding = tf.Variable(tf.random.normal([1, self.hidden_size], 0.0, 0.01),
                                       name='reward_embedding')
        # cls_embedding = tf.Variable(tf.random.normal([1, self.hidden_size], 0.0, 0.01),
        #                                name='cls_embedding')
